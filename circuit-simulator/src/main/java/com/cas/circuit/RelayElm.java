...
class RelayElm extends CircuitElm {
	...
	void draw(Graphics g) {
		...
		for (i = 0; i != 2; i++) {
			...
			CircuitUtil.drawThickLine(g, coilLeads[i], coilPosts[i]);
		}
		...
		for (i = 0; i != poleCount; i++) {
			if (i == 0) {
				CircuitUtil.interpPoint(point1, point2, lines[i * 2], .5, openhs * 2 + 5 * dsign - i * openhs * 3);
			} else {
				CircuitUtil.interpPoint(point1, point2, lines[i * 2], .5, (int) (openhs * (-i * 3 + 3 - .5 + d_position)) + 5 * dsign);
			}
			CircuitUtil.interpPoint(point1, point2, lines[i * 2 + 1], .5, (int) (openhs * (-i * 3 - .5 + d_position)) - 5 * dsign);
		...
		for (p = 0; p != poleCount; p++) {
			...
			for (i = 0; i != 3; i++) {
				...
				CircuitUtil.drawThickLine(g, swposts[p][i], swpoles[p][i]);
			}

			CircuitUtil.interpPoint(swpoles[p][1], swpoles[p][2], ptSwitch[p], d_position);
			// setVoltageColor(g, volts[nSwitch0]);
			g.setColor(Color.lightGray);
			CircuitUtil.drawThickLine(g, swpoles[p][0], ptSwitch[p]);
	...
	@Override
	String dump() {
		return super.dump() + " " + poleCount + " " + inductance + " " + coilCurrent + " " + r_on + " " + r_off + " " + onCurrent + " " + coilR;
	}
	...
	void getInfo(String arr[]) {
		...
		int ln = 1;
		for (i = 0; i != poleCount; i++)
			arr[ln++] = "I" + (i + 1) + " = " + CircuitUtil.getCurrentDText(switchCurrent[i]);
		arr[ln++] = "coil I = " + CircuitUtil.getCurrentDText(coilCurrent);
		arr[ln++] = "coil Vd = " + CircuitUtil.getVoltageDText(volts[nCoil1] - volts[nCoil2]);
	}
	...
	void setPoints() {
		...
		for (i = 0; i != poleCount; i++) {
			...
			CircuitUtil.interpPoint(lead1, lead2, swpoles[i][0], 0, -openhs * 3 * i);
			CircuitUtil.interpPoint(lead1, lead2, swpoles[i][1], 1, -openhs * 3 * i - openhs);
			CircuitUtil.interpPoint(lead1, lead2, swpoles[i][2], 1, -openhs * 3 * i + openhs);
			CircuitUtil.interpPoint(point1, point2, swposts[i][0], 0, -openhs * 3 * i);
			CircuitUtil.interpPoint(point1, point2, swposts[i][1], 1, -openhs * 3 * i - openhs);
			CircuitUtil.interpPoint(point1, point2, swposts[i][2], 1, -openhs * 3 * i + openhs);
		}
		...
		int x = ((flags & FLAG_SWAP_COIL) != 0) ? 1 : 0;
		CircuitUtil.interpPoint(point1, point2, coilPosts[0], x, openhs * 2);
		CircuitUtil.interpPoint(point1, point2, coilPosts[1], x, openhs * 3);
		CircuitUtil.interpPoint(point1, point2, coilLeads[0], .5, openhs * 2);
		CircuitUtil.interpPoint(point1, point2, coilLeads[1], .5, openhs * 3);
