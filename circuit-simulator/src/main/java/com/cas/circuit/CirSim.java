...
//	hintItem2
	...
	private void initUI(boolean printable, boolean convention, boolean euro) {
		...
		MenuBar mb = null;
		if (useFrame) mb = new MenuBar();
		Menu m = new Menu("File");
		if (useFrame) mb.add(m);
		else
			mainMenu.add(m);
		...
		if (useFrame) mb.add(circuitsMenu);
		else
		...
		if (useFrame) main.add(new Label(""));
	...
	void handleResize() {
		...
		if (dx + minx < 0) dx = gridMask & (-minx);
		if (dy + miny < 0) dy = gridMask & (-miny);
		for (i = 0; i != elmList.size(); i++) {
			CircuitElm ce = getElm(i);
			ce.move(dx, dy);
		}
	...
	public void updateCircuit(Graphics realg) {
		...
		if (editDialog != null && editDialog.elm instanceof CircuitElm) mouseElm = (CircuitElm) (editDialog.elm);
		...
		if (!stoppedCheck.getState()) {
			...
			if (lastTime != 0) {
				...
				if (!conventionCheckItem.getState()) CircuitElm.currentMult = -CircuitElm.currentMult;
			}
		...
		if (tempMouseMode == MODE_DRAG_ROW || tempMouseMode == MODE_DRAG_COLUMN || tempMouseMode == MODE_DRAG_POST || tempMouseMode == MODE_DRAG_SELECTED) for (i = 0; i != elmList.size(); i++) {
			CircuitElm ce = getElm(i);
		...
		if (nodeList != null) for (i = 0; i != nodeList.size(); i++) {
			...
			if (!cn.internal && cn.links.size() == 1) {
				...
				for (j = 0; j != elmList.size(); j++) { // TODO: (hausen)
					...
					if (cnl.elm != ce && getElm(j).boundingBox.contains(cn.x, cn.y)) bb++;
				}
				if (bb > 0) {
		...
		/*
		 * if (mouseElm != null) { g.setFont(oldfont); g.drawString("+", mouseElm.x+10, mouseElm.y); }
		 */
		if (dragElm != null && (dragElm.x != dragElm.x2 || dragElm.y != dragElm.y2)) dragElm.draw(g);
		g.setFont(oldfont);
		int ct = scopeCount;
...
//		显示提示信息
		...
		frames++;
		/*
		 * g.setColor(Color.white); g.drawString("Framerate: " + framerate, 10, 10); g.drawString("Steprate: " + steprate, 10, 30); g.drawString("Steprate/iter: " + (steprate/getIterCount()), 10, 50); g.drawString("iterc: " + (getIterCount()), 10, 70);
		 */
	...
	void setupScopes() {
		...
		for (i = 0; i < scopeCount; i++) {
			if (locateElm(scopes[i].elm) < 0) scopes[i].setElm(null);
...
//			因电容是电子设备中大量使用的电子元件之一，所以广泛应用于隔直、耦合、旁路、滤波、调谐回路、能量转换、控制电路等方面。
	...
	void analyzeCircuit() {
		...
		for (i = 0; i != elmList.size(); i++) {
			...
			if (ce instanceof RailElm) gotRail = true;
			if (volt == null && ce instanceof VoltageElm) volt = ce;
		}
		...
		for (i = 0; i != matrixSize; i++) {
			...
			/*
			 * if (qp != -1 && circuitRowInfo[qp].lsChanges) { System.out.println("lschanges"); continue; } if (qm != -1 && circuitRowInfo[qm].lsChanges) { System.out.println("lschanges"); continue; }
			 */
			if (j == matrixSize) {
				if (qp == -1) {
		...
		/*
		 * System.out.println("matrixSize = " + matrixSize); for (j = 0; j != circuitMatrixSize; j++) { System.out.println(j + ": "); for (i = 0; i != circuitMatrixSize; i++) System.out.print(circuitMatrix[j][i] + " "); System.out.print("  " + circuitRightSide[j] + "\n"); } System.out.print("\n");
		 */
		...
		for (i = 0; i != matrixSize; i++) {
			...
			for (j = 0; j != matrixSize; j++) {
				...
				if (ri.type == RowInfo.ROW_CONST) newrs[ii] -= ri.value * circuitMatrix[i][j];
				else
		...
		/*
		 * System.out.println("matrixSize = " + matrixSize + " " + circuitNonLinear); for (j = 0; j != circuitMatrixSize; j++) { for (i = 0; i != circuitMatrixSize; i++) System.out.print(circuitMatrix[j][i] + " "); System.out.print("  " + circuitRightSide[j] + "\n"); } System.out.print("\n");
		 */
	...
	void calcCircuitBottom() {
		...
		for (i = 0; i != elmList.size(); i++) {
			...
			if (bottom > circuitBottom) circuitBottom = bottom;
		}
	}
	...
	class FindPathInfo {
		...
		boolean findPath(int n1, int depth) {
			...
			for (i = 0; i != elmList.size(); i++) {
				...
				if (type == VOLTAGE) {
					if (!(ce.isWire() || ce instanceof VoltageElm)) continue;
				}
	...
	void runCircuit() {
		...
		for (iter = 1;; iter++) {
			...
			lit = tm;
			if (iter * 1000 >= steprate * (tm - lastIterTime) || (tm - lastFrameTime > 500)) break;
		}
		lastIterTime = lit;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == optionsItem) doEdit(new EditOptions(this));
	...
	void stackScope(int s) {
		...
		if (scopes[s].position == scopes[s - 1].position) return;
	...
	void doImport() {
		if (impDialog == null) impDialog = ImportExportDialogFactory.Create(this, ImportExportDialog.Action.IMPORT);
	...
	void doExport(boolean url) {
		...
		if (url) dump = baseURL + "#" + URLEncoder.encode(dump);
		if (expDialog == null) {
	...
	URL getCodeBase() {
		try {
			if (applet != null) return applet.getCodeBase();
			File f = new File(".");
...
//				tint == '$'
				try {
					...
					if (tint >= '0' && tint <= '9') tint = new Integer(type).intValue();
...
//		右侧设置面板 <<<<<<<<<<
	...
	boolean doSwitch(int x, int y) {
		...
		se.toggle();
		if (se.momentary) heldSwitchElm = se;
		needAnalyze();
		return true;
	}
	...
	public void mouseDragged(MouseEvent e) {
		...
		if ((e.getModifiers() & InputEvent.BUTTON3_MASK) != 0) {
			...
			if ((ex & (InputEvent.META_DOWN_MASK | InputEvent.SHIFT_DOWN_MASK | InputEvent.CTRL_DOWN_MASK | InputEvent.ALT_DOWN_MASK)) == 0) return;
		}
		if (!circuitArea.contains(e.getX(), e.getY())) return;
		if (dragElm != null) dragElm.drag(e.getX(), e.getY());
	...
	void dragRow(int x, int y) {
		...
		for (i = 0; i != elmList.size(); i++) {
			CircuitElm ce = getElm(i);
			if (ce.y == dragY) ce.movePoint(0, 0, dy);
			if (ce.y2 == dragY) ce.movePoint(1, 0, dy);
		}
		removeZeroLengthElements();
	}
	...
	void dragColumn(int x, int y) {
		...
		for (i = 0; i != elmList.size(); i++) {
			CircuitElm ce = getElm(i);
			if (ce.x == dragX) ce.movePoint(0, dx, 0);
			if (ce.x2 == dragX) ce.movePoint(1, dx, 0);
		}
		removeZeroLengthElements();
	}
	...
	boolean dragSelected(int x, int y) {
		...
		for (i = 0; i != elmList.size(); i++) {
			CircuitElm ce = getElm(i);
			if (ce.isSelected() && !(ce instanceof GraphicElm)) break;
		}
		if (i != elmList.size()) {
			x = snapGrid(x);
			y = snapGrid(y);
	...
	public void mouseMoved(MouseEvent e) {
		...
		for (i = 0; i != elmList.size(); i++) {
			...
			if (ce.boundingBox.contains(x, y)) {
				...
				if (ce.getPostCount() == 0) mouseElm = ce;
			}
		}
		scopeSelected = -1;
		if (mouseElm == null) {
			...
			for (i = 0; i != mouseElm.getPostCount(); i++) {
				...
				if (distanceSq(pt.x, pt.y, x, y) < 26) mousePost = i;
			}
		}
		if (mouseElm != origMouse) cv.repaint();
	}
	...
	void doMainMenuChecks(Menu m) {
		int i;
		if (m == optionsMenu) return;
		for (i = 0; i != m.getItemCount(); i++) {
			MenuItem mc = m.getItem(i);
			if (mc instanceof Menu) doMainMenuChecks((Menu) mc);
	...
	public void mouseReleased(MouseEvent e) {
		...
		if (dragElm != null) {
			...
			if (dragElm.x == dragElm.x2 && dragElm.y == dragElm.y2) dragElm.delete();
			else {
	...
	void pushUndo() {
		...
		if (undoStack.size() > 0 && s.compareTo(undoStack.lastElement()) == 0) return;
		undoStack.add(s);
		enableUndoRedo();
	}
	...
	void setMouseMode(int mode) {
		mouseMode = mode;
		if (mode == MODE_ADD_ELM) cv.setCursor(new Cursor(Cursor.CROSSHAIR_CURSOR));
		else
	...
	void doPaste() {
		...
		if (oldbb != null && newbb != null && oldbb.intersects(newbb)) {
			...
			if (spacew > spaceh) dx = snapGrid(oldbb.x + oldbb.width - newbb.x + gridSize);
			else
