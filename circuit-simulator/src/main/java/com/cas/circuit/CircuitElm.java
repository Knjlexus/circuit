...
//	adj. 对角线的; 斜的; 斜线的; 斜纹的;
	...
	void drawPost(Graphics g, int x0, int y0, int n) {
		if (sim.dragElm == null && !needsHighlight() && sim.getCircuitNode(n).links.size() == 2) return;
		if (sim.mouseMode == CirSim.MODE_DRAG_ROW || sim.mouseMode == CirSim.MODE_DRAG_COLUMN) return;
		drawPost(g, x0, y0);
	}
	...
	void drawValues(Graphics g, String s, double hs) {
		...
		if (dpx == 0) {
			...
			int xx = xc + abs(dpx) + 2;
			if (this instanceof VoltageElm || (x < x2 && y > y2)) xx = xc - (w + abs(dpx) + 2);
	...
	double updateDotCount(double cur, double cc) {
		if (sim.stoppedCheck.getState()) return cc;
	...
	void doDots(Graphics g) {
		updateDotCount();
		if (sim.dragElm != this) drawDots(g, point1, point2, curcount);
	}
	...
	void setVoltageColor(Graphics g, double volts) {
		...
		if (c < 0) c = 0;
		if (c >= colorScaleCount) c = colorScaleCount - 1;
		g.setColor(colorScale[c]);
	}

	void setPowerColor(Graphics g, boolean yellow) {
		/*
		 * if (conductanceCheckItem.getState()) { setConductanceColor(g, current/getVoltageDiff()); return; }
		 */
		if (!sim.powerCheckItem.getState()) return;
	...
	void setPowerColor(Graphics g, double w0) {
		 ...
		 */
		if (w0 > 0) g.setColor(new Color(rg, b, b));
		else
